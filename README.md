...
- [SO-ARM101 VLA Control System](#so-arm101-vla-control-system)
  - [목차](#목차)
  - [아키텍처](#아키텍처)
    - [시뮬레이션 경로 (LeIsaac + Isaac Sim)](#시뮬레이션-경로-leisaac--isaac-sim)
    - [실기기 경로 (LeRobot)](#실기기-경로-lerobot)
  - [환경 설정](#환경-설정)
    - [사전 요구사항](#사전-요구사항)
    - [핵심 의존성](#핵심-의존성)
    - [설치 단계](#설치-단계)
    - [선택 사항](#선택-사항)
  - [사용법](#사용법)
    - [1. Isaac Sim 텔레오퍼레이션 (시뮬레이션)](#1-isaac-sim-텔레오퍼레이션-시뮬레이션)
    - [2. 실기기 텔레오퍼레이션 + 데이터 수집](#2-실기기-텔레오퍼레이션--데이터-수집)
    - [3. 정책 학습](#3-정책-학습)
      - [ACT (경량·빠른 학습)](#act-경량빠른-학습)
      - [SmolVLA (VLA·언어 조건부)](#smolvla-vla언어-조건부)
    - [4. 파인튜닝 (사전 학습 체크포인트 기반)](#4-파인튜닝-사전-학습-체크포인트-기반)
    - [5. Hub 업로드](#5-hub-업로드)
  - [정책 비교](#정책-비교)
  - [출력 디렉토리 구조](#출력-디렉토리-구조)
  - [하드웨어 구성](#하드웨어-구성)
  - [Troubleshooting](#troubleshooting)
    - [카메라 대역폭 제한](#카메라-대역폭-제한)
      ...
      - [테스트 결과](#테스트-결과)
      - [해결 방법](#해결-방법)
      - [MJPG 실제 적용 여부 확인 방법](#mjpg-실제-적용-여부-확인-방법)
      - [USB 버전 확인 방법](#usb-버전-확인-방법)
    - [h5py와 Isaac Sim의 HDF5 ABI 불일치 (Windows)](#h5py와-isaac-sim의-hdf5-abi-불일치-windows)
      - [원인](#원인-1)
      - [해결 방법](#해결-방법-1)
      - [확인 방법](#확인-방법)
      - [DLL 버전 비교 (디버깅용)](#dll-버전-비교-디버깅용)
    - [시뮬레이션 기동 시 무시해도 되는 로그](#시뮬레이션-기동-시-무시해도-되는-로그)
      - [로그 카테고리별 해석](#로그-카테고리별-해석)
      - [실제로 주의해야 할 로그](#실제로-주의해야-할-로그)
  - [Reference](#reference)
...
- **Windows 10 / 11** — Isaac Sim 5.1 Windows 빌드 기준
